...
**Status:** SYNCED — this content model reflects the **built Industrial & IoT HTML page** (the source of truth; `Hana Industrial & IoT Hub (standalone).html`). Copy below is transcribed from that page's content model. The built page renders a "capabilities behind this market" card row; per Rupert that slot is a **products-built** section, not capability links — so it is modelled here as a "Product examples" section, and no capability-link card row appears. Run `hana:brand-review` and clear the ⚑ items before publishing.
...
Industrial buyers care about long product lifecycles, harsh-environment reliability and supply continuity — not consumer-style volume ramps. Lead with ruggedised build (conformal coating, wide temperature ranges), long-run programme support and obsolescence discipline. Distinct from the Consumer Electronics hub (volume/miniaturisation) and Automotive hub (vehicle qualification).
...
- **Intro (~50 words):** The controllers, connected devices and power electronics behind industrial equipment — built for the harsh environments, long production runs and supply continuity these programmes demand. We assemble the board and, where you need it, the finished unit.
...
- **Heading:** Ruggedised build, held steady over long production runs
- **Body (~100 words):** Industrial and IoT products are not consumer volume ramps. They run in cabinets, plants and outdoor enclosures, and their programmes stay in production far longer than consumer ones do. That changes what a manufacturer has to be good at: ruggedised assembly such as conformal and parylene coating, disciplined change control across long production runs, and the ability to keep building a product after its first components go end-of-life. Those are the processes we run, and we run the same qualified ones across more than one country so a programme has somewhere to go if it needs to.
- **Three points:**
  1. **Ruggedised build** — Conformal coating, mixed-technology boards and through-hole for heavy connectors — assembly choices made for heat, vibration and moisture, not the bench.
  2. **Long-programme support** — Change control and obsolescence discipline over runs that last years, so the product you qualify is the product that keeps shipping.
...
- **PCBA & box build** → `/markets/industrial-iot/pcba-box-build/` — Ruggedised board assembly through to the tested, finished unit — conformal and parylene coating, mixed-technology boards and functional test for controls and equipment in demanding environments. · _Conformal-coated industrial board_
...
- **What does Hana build for industrial and IoT?** Ruggedised PCBAs and full box-build products, complete connected IoT devices, RFID asset-tracking tags, and power modules for industrial energy and drives. In most cases we can build the complete unit, not only the board.
- **How does Hana support long-running industrial programmes?** Through change control and obsolescence management across production runs that last years, so the build stays consistent as components move through their own life cycles — and through ruggedised assembly options such as conformal and parylene coating where the environment demands them.
...
- **What ruggedised finishing can Hana apply?** Conformal and parylene coating and harsh-environment finishing are available within our industrial box-build scope. The finish set is selected per programme and confirmed per site during the programme review.
...
- Which sites serve industrial programmes
...
- ⚑ The built HTML page currently renders a "capabilities behind this market" card row — per Rupert this should be the products-built section instead (modelled above as "Product examples"). Update the built page so its render matches this model (drop the capability-link cards).
