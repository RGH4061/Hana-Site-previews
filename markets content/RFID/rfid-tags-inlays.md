...
keywords: "RFID tag manufacturer, RFID inlay manufacturer, UHF inlay, HF inlay, RFID strap, on-metal RFID tag, temperature sensing RFID tag, battery assisted passive tag"
---

# RFID tags and inlays — UHF, HF and LF
...
<!-- VERIFY BEFORE PUBLISH (hand to Hana ops):
  ...
  - Throughput / capability-extent figures safe to publish (none asserted here — no line speeds,
    no annual volumes).
  - Site framing (currently China + USA — Jiaxing + Ohio).
  - ⚑ "Six automated inlay lines at Jiaxing" — from the fact pack; confirm the current line count
    before it goes live (same flag as the RFID hub page).
  - ⚑ ARC (Auburn) certification — copy asserts certification in the USA and China but deliberately
    omits the "192+ listings across 26 certified tags" figures. Confirm the certification is
    current and agree the public wording (and whether the figures may be cited) before publish.
  - ⚑ LF availability: the H1 and meta description (both from the page plan) name LF, but the fact
    pack lists UHF and HF for inlays and tags. Confirm LF tag/inlay production or amend the H1.
  - Current production status for temperature-sensing tags, battery-assisted passive builds and
    on-metal formats — confirm each is offered rather than developmental.
  - Whether straps are supplied as a standalone merchant product or only inside Hana's own builds.
